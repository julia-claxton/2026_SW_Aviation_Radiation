...
%DIF DEL claxton2026_aviation_revision_0.tex   Sun Apr 19 09:56:02 2026
%DIF ADD claxton2026_aviation_revision_1.tex   Sun Apr 19 11:58:39 2026
...
\begin{keypoints}
  \item An existing simulation is used to predict radiation doses due to galactic cosmic rays (GCRs) and relativistic electron precipitation (REP).
  \item Our predictions of GCR dose rates are in statistical agreement with data, but do not explain a small population of higher dose rates.
  \item \DIFdelbegin \DIFdel{We find that }\DIFdelend REP-induced dose rates are unlikely to account for the difference between GCR modeling and aircraft dosimetry data.
\end{keypoints}
...
\section{Background}
There are two well-known sources of background radiation doses at aviation altitudes: SEPs and GCRs \cite{mertens2013}. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend state-of-the-art model for predicting radiation doses across the Earth's atmosphere, NAIRAS \cite{mertens2013}, accounts for both of these radiation types. NAIRAS \DIFaddbegin \DIFadd{version 1 accounts for SEP and GCRs, while NAIRAS version 3 also includes effects due to trapped (but not precipitating) energetic particles \mbox{%DIFAUXCMD
\cite{mertens2025}}\hskip0pt%DIFAUXCMD
. NAIRAS }\DIFaddend parameterizes incoming GCR spectra using high-latitude neutron counts. Neutrons are produced as a cascade product from GCRs incident on the atmosphere, and thus their detection rate at the ground serves as an indirect measure of GCR incidence. NAIRAS then propagates the derived GCR spectrum through the heliosphere, magnetosphere, and atmosphere to produce dose rate predictions. We do not discuss SEP contributions to dose rate in this paper, as there are no ARMAS data records correlated with SEP events. NAIRAS \DIFdelbegin \DIFdel{, currently in version 3, }\DIFdelend was validated with aircraft dosimeter data from the RAD-X balloon campaign showing agreement within the measurement uncertainty of $\pm 30\%$.
\DIFdelbegin \DIFdel{In this work, we utilize NAIRAS version 3 values that are provided with ARMAS data.
}\DIFdelend 
...
To model the dose rate induced by GCRs at aviation altitudes, we needed to calculate the dose rate induced by a given set of precipitating particles. For protons, electrons, \DIFaddbegin \DIFadd{muons, }\DIFaddend and alpha particles, we utilized the fact that particle flux multiplied by stopping power in a material of interest (e.g. silicon) yields a dose rate in that material. The cumulative dose rate induced by protons, electrons, \DIFaddbegin \DIFadd{muons, }\DIFaddend and alpha particles at altitude $h$ is given by
%
\begin{equation}
  \text{DR}\DIFdelbegin \DIFdel{_{\text{p}^+,\text{e}^-,\alpha}}\DIFdelend \DIFaddbegin \DIFadd{_{\text{p}^+,\text{e}^-,\mu^-,\alpha}}\DIFaddend (h) = \sum_i \sum_E \Phi_i(E,h) \cdot \text{S}_i(E,h),
\end{equation}
%
\noindent where $i$ represents particle species, $E$ is the energy of an incident particle, $\Phi_i(E)$ is the flux of species $i$ at energy $E$, $S$ is the stopping power of silicon for species $i$ at energy $E$, and DR\DIFdelbegin \DIFdel{$_{\text{p}^+,\text{e}^-,\alpha}$ }\DIFdelend \DIFaddbegin \DIFadd{$_{\text{p}^+,\text{e}^-,\mu^-,\alpha}$ }\DIFaddend is the resultant cumulative dose rate in silicon due to these particles.
...
%
\begin{equation}
  \text{DR}(h) = \text{DR}\DIFdelbegin \DIFdel{_{\text{p}^+,\text{e}^-,\alpha}}\DIFdelend \DIFaddbegin \DIFadd{_{\text{p}^+,\text{e}^-,\mu^-,\alpha}}\DIFaddend (h) + \text{DR}_\gamma(h).
\end{equation}
%
Stopping power tables were sourced from NIST for protons ($1$~keV to $10$~GeV), electrons ($10$~keV to $1$~GeV), and alphas ($1$~keV to $1$~GeV) \cite{nist-stoppingpower}. \DIFaddbegin \DIFadd{Stopping power tables for muons ($10$~MeV to $100$~TeV) were sourced from \mbox{%DIFAUXCMD
\citeA{groom2001}}\hskip0pt%DIFAUXCMD
. }\DIFaddend X-ray mass energy-absorption coefficients were sourced from NIST for photons between $1$~keV and $20$~MeV \cite{nist-xrayattenuation}. The value of $\mu_\text{en}$ for photons above $20$~MeV was taken as the value at $20$~MeV, as $\mu_\text{en}$ does not change significantly with energy above $\sim 5$~MeV in the NIST database. This extrapolation added approximately $0.6$~{\ugy}/hr to our predicted GCR dose rates at aviation altitudes. The stopping power and X-ray mass energy-absorption coefficient values used in this work are shown in Figure \ref{figure:stopping_power}.

\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures_rev0/stopping_power.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/stopping_power.pdf}
  \DIFaddendFL \caption{
...
Since the calculation of dose rate is species- and energy-dependent, we need to record an energy spectrum for every species of interest at every location where we want to calculate a dose rate in our simulation. Since we are interested in the altitude profile of EPP-induced dose rates, we configure SEPIIDA to record the energy spectrum of all electrons, \DIFaddbegin \DIFadd{muons, }\DIFaddend protons, alphas, and photons passing through a set of recording altitudes. These recording altitudes are spaced from sea level to $100$~km altitude in $1$~km increments. After a simulation run is completed, the counts recorded in every energy bin at each recording altitude are divided by the total number of input particles. This division produces a normalized response table giving the number of counts of a given particle that are produced by one precipitating primary particle over the energies and altitudes of interest. An example of these normalized response spectra is shown in Figure \ref{figure:example_histograms}. The spectra shown are the response to a monoenergetic input beam of $10^5$ protons each with an energy of \DIFdelbegin \DIFdel{$1.1$}\DIFdelend \DIFaddbegin \DIFadd{$11$}\DIFaddend ~GeV injected into the simulation at $450$~km with an initial velocity randomly sampled from an isotropically downgoing distribution. Secondary alpha fluxes are negligible and are not shown in the figure. The primary protons are evident as the white line in the \DIFdelbegin \DIFdel{leftmost }\DIFdelend \DIFaddbegin \DIFadd{top left }\DIFaddend panel, with the primary energy decreasing at low altitudes. \DIFaddbegin \DIFadd{A backscattered population of protons is also visible with lower energy and flux than the precipitating primaries. }\DIFaddend The $511$~keV photon line due to pair annihilation is evident in the \DIFdelbegin \DIFdel{rightmost }\DIFdelend \DIFaddbegin \DIFadd{bottom left }\DIFaddend panel.

\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures_rev0/example_histograms.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{figures/example_histograms.pdf}
  \DIFaddendFL \caption{
    Example simulated energy spectra as a function of altitude for protons (\DIFaddbeginFL \DIFaddFL{top }\DIFaddendFL left), electrons (\DIFdelbeginFL \DIFdelFL{center}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top right}\DIFaddendFL ), \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL photons (\DIFaddbeginFL \DIFaddFL{bottom left), and muons (bottom }\DIFaddendFL right). These spectra are the response to an input beam of $10^5$ protons at \DIFdelbeginFL \DIFdelFL{$1.1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$11$}\DIFaddendFL ~GeV injected with isotropic downgoing incidence into an MSIS atmosphere at $450$~km.
  }
...
\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures_rev0/gcr_spectrum.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/gcr_spectrum.pdf}
  \DIFaddendFL \caption{
...
In Figure \ref{figure:case_studies}, we plot four example \DIFdelbegin \DIFdel{conjunctions }\DIFdelend \DIFaddbegin \DIFadd{cases }\DIFaddend showing a) agreement between SEPIIDA, NAIRAS \DIFaddbegin \DIFadd{v1}\DIFaddend , and ARMAS; b) disagreement between NAIRAS \DIFaddbegin \DIFadd{v1 }\DIFaddend and ARMAS, but agreement between ARMAS and SEPIIDA; c) small excess dose rates in ARMAS above the SEPIIDA prediction; and d) large excess dose rates in ARMAS above the SEPIIDA prediction. Error bars on ARMAS data represent the $1\sigma$ uncertainty, which is $20\%$. \DIFdelbegin \DIFdel{SEPIIDA predictions of dose rate are derived from }\DIFdelend \DIFaddbegin \DIFadd{When comparing an AMS-02-derived dose rate to ARMAS-measured dose rates, we only required that the }\DIFaddend AMS-02 \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{spectra and ARMAS dose rates were }\DIFaddend recorded on the \DIFdelbegin \DIFdel{date of each panel. Note that }\DIFdelend \DIFaddbegin \DIFadd{same day. }\DIFaddend AMS-02 \DIFaddbegin \DIFadd{provides spectra at a daily cadence, thus no stricter conjunction requirements could be used.
}

\begin{figure}[H]
  \centering
  \noindent\includegraphics[width=\textwidth]{figures/case_studies.pdf}
  \caption{
      \DIFaddFL{Four example conjunction dates between ARMAS and AMS-02 showing the ARMAS-measured dose rate (black dots), NAIRAS v1-predicted GCR dose rate (blue crosses), and our predicted GCR dose rate based on AMS-02 data (pink line).
  }}
  \label{figure:case_studies}
\end{figure}

\DIFadd{Since AMS-02 }\DIFaddend data is provided at a daily cadence for the ISS's orbit at $51\mathdeg$ inclination, \DIFdelbegin \DIFdel{spanning a }\DIFdelend \DIFaddbegin \DIFadd{AMS-02 spectra spans a larger }\DIFaddend range of latitudes \DIFdelbegin \DIFdel{, while the ARMAS data}\DIFdelend \DIFaddbegin \DIFadd{than ARMAS data, which }\DIFaddend is primarily limited to the mid-latitudes of the continental United States. Thus, the AMS-02 data experiences a wider range of cutoff rigidities than ARMAS and may record higher fluxes at low rigidities than ARMAS would experience, causing another potential overestimation of GCR dose rates. Once again, we accept this overestimation in order to determine if the excess dose rates can be explained by an upper-bound case for GCR dose rates.

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \noindent\includegraphics[width=\textwidth]{figures_rev0/case_studies.pdf}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Four example conjunction dates between ARMAS and AMS-02 showing the ARMAS-measured dose rate (black dots), NAIRAS-predicted GCR dose rate (blue crosses), and our predicted GCR dose rate based on AMS-02 data (pink line).
  }}
  %DIFAUXCMD
%DIFDELCMD < \label{figure:case_studies}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend With the ability to compute a dose rate profile from an AMS-02 spectrum, we next compare our predicted GCR dose rates to ARMAS-measured dose rates for every day where AMS-02 and ARMAS both recorded data. For each day, we subtract our predicted GCR-induced dose rate from the ARMAS-measured dose rate to produce a residual dose rate. The GCR-induced dose rate was taken at the same altitude of each ARMAS data record. The results of this comparison are shown in Figure \ref{figure:armas_residuals}, along with the results of the same procedure using NAIRAS version \DIFdelbegin \DIFdel{3 }\DIFdelend \DIFaddbegin \DIFadd{1 }\DIFaddend predictions of GCR dose rate for reference.

\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures_rev0/armas_residuals.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/armas_residuals.pdf}
  \DIFaddendFL \caption{
      Left: Residuals between our GCR dose rate predictions and ARMAS data for all conjunction dates with AMS-02. Right: Residuals between NAIRAS \DIFaddbeginFL \DIFaddFL{v1 }\DIFaddendFL predictions and ARMAS data for the same set of dates.
  }
...
\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\textwidth]{figures_rev0/elfin_example_data.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{figures/elfin_example_data.pdf}
  \DIFaddendFL \caption{
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{cccccc}
    \DIFaddendFL Percentile & Date (YYYY-MM-DD) & Time (UTC)   & Satellite \DIFaddbeginFL & \DIFaddFL{L-Shell }& \DIFaddFL{MLT (hh:mm) }\DIFaddendFL \\
    \hline
    $99.9$     & 2022-02-04        & 15:17:32.015 & ELFIN-B \DIFaddbeginFL & \DIFaddFL{5.4 }& \DIFaddFL{05:00 }\DIFaddendFL \\
    $98$       & 2021-11-07        & 02:12:46.867 & ELFIN-B \DIFaddbeginFL & \DIFaddFL{3.7 }& \DIFaddFL{21:48 }\DIFaddendFL \\
    $90$       & 2020-10-22        & 23:04:22.315 & ELFIN-A \DIFaddbeginFL & \DIFaddFL{14.5 }& \DIFaddFL{20:39
  }\DIFaddendFL \end{tabular}
  \caption{
    Timestamps\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL satellite identification information\DIFaddbeginFL \DIFaddFL{, and geomagnetic coordinates }\DIFaddendFL for our selected ELFIN-measured spectra.
  }
...
\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures_rev0/electron_spectrum.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/electron_spectrum.pdf}
  \DIFaddendFL \caption{
...
\begin{figure}[H]
  \centering
  \noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.85\textwidth]{figures_rev0/excess_dose_comparison.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.85\textwidth]{figures/excess_dose_comparison.pdf}
  \DIFaddendFL \caption{
...
Previous modeling efforts also support this conclusion. As previously discussed, \citeA{xu2021} predicted that a flux of $10^8$~electrons$~\cdot~$m$^{-2}~\cdot~$s$^{-1}$ of $10$~MeV electrons, a flux orders of magnitude higher than typical observed electron fluxes at LEO, would induce a dose rate of just $0.05$~{\ugy}/hr at an altitude of $11$~km. Meanwhile, typical excess dose rates are on the order of $1$~{\ugy}/hr (Figure \ref{figure:rep:excess_dose_comparison}). Thus, their work predicts that an extreme flux of ultrarelativistic electrons could contribute only up to about $5\%$ of a typical excess dose rate. In the case studies presented by \citeA{mcmurchie2022}, the highest fraction of total (not excess) dose rate explained by REP was $3.66\%$. To explain the excess dose rates measured by ARMAS, REP-induced dose rates would have to make up about $10\%$ to $50\%$ of the total dose rate. \DIFdelbegin \DIFdel{The results of both of these studies }\DIFdelend \DIFaddbegin \DIFadd{These modeling results }\DIFaddend suggest that REP is unlikely to be able to generate dose rates on the order of the observed excess dose rates.

We recognize the compelling correlation between hiss wave activity and excess dose rates presented in \citeA{aryan2025b}. The goal of this study is not to explain that correlation, but simply to quantitatively constrain the hypothesis that REP is responsible for that correlation. Between our analysis and previous literature, we believe it is unlikely that relativistic electron precipitation is the direct cause of the discrepancies between GCR dose rate predictions and dose rates measured by the ARMAS system\DIFaddbegin \DIFadd{.
}

\DIFadd{It is also worth noting that there are other effects that may contribute to the discrepancy between ARMAS measurements and  predictions. Firstly, both NAIRAS and SEPIIDA predict GCR-induced dose rates with no shielding. In reality, the fuselage of an aircraft provides some shielding from incident particles and may also generate additional particle showers, potentially affecting the dose rates measured from inside an aircraft cabin. Furthermore, NAIRAS parameterizes altitude by barometric pressure, while the ARMAS dataset utilizes GPS data to calculate altitude. Altitudes computed with different methods may differ slightly, adding another potential source of discrepancy between the datasets}\DIFaddend . 
...
ELFIN data \cite{angelopoulos2020} is available for free at \url{https://data.elfin.ucla.edu/ela/}. Geant4 \cite{geant4} is available for free from \url{https://geant4.web.cern.ch/download/11.3.2.html}. The SEPIIDA code used to generate lookup tables is available for free at \url{https://doi.org/10.5281/zenodo.18473010}. The atmospheric response lookup tables generated by the Geant4 simulation are available for free at \url{https://doi.org/10.5281/zenodo.18473107}. All code used to perform the analysis and generate figures for this paper is available for free at \url{https://doi.org/10.5281/zenodo.18473200} . Analysis was performed using the Julia coding language \cite{julia-language}, NumPy \cite{numpy}, SpacePy \cite{spacepy1, spacepy2}, and Matplotlib \cite{matplotlib}.
